  ...
  for(ix=0;ix<Lx;ix++) //for each cell
    for(iy=0;iy<Ly;iy++)
      for(i=0;i<Q;i++){ //on each direction
        n0=n(ix,iy,i);
        f[n0] = feq(rho0,Jx0,Jy0,i,C(ix,iy));
      }
}  
  ...
  for(ix=0;ix<Lx;ix++) //for each cell
    for(iy=0;iy<Ly;iy++){
      ...
      for(i=0;i<Q;i++){ //for each velocity vector
        n0=n(ix,iy,i);
        fnew[n0]=UmUtau*f[n0]+Utau*feq(rho0,Jx0,Jy0,i,C(ix,iy));
      }
    }  
}
void LatticeBoltzmann::ImposeFields(int t){
  ...
  ix=0;
  for(iy=0;iy<Ly;iy++){
    omega=2*M_PI/lambda*C(ix,iy);
    rho0=10*sin(omega*t); Jx0=Jx(ix,iy,false); Jy0=Jy(ix,iy,false);
    for(i=0;i<Q;i++){
      n0=n(ix,iy,i);
      fnew[n0]=feq(rho0,Jx0,Jy0,i,C(ix,iy));
  }
  }
...
double LatticeBoltzmann::C(int ix,int iy){
  int ixLeft= 64;
  int WC = 32;
  double f = (tanh(ix-(ixLeft-WC))+1)*0.3/2.0 +1;
  double g = (-tanh(ix-(ixLeft+WC))-1)*0.3/2.0;
  double n = f+g;
