...
public:
  ...
  vector3D Jprime(vector3D & E0,double prefactor0){return E0*prefactor0;};
  vector3D Eprime(vector3D & E0,vector3D & Jprime0,double epsilonr0){return E0-Jprime0*(Mu0/(4*epsilonr0));};
  vector3D Dprime(vector3D & Eprime0,double epsilonr0){return Eprime0/epsilonr0;};
  //Equilibrium Functions
  double feq(vector3D & Jprime0,vector3D & Eprime0,vector3D & B0,double Epsilonr,double Mur,int r,int p,int i,int j);
...
//---------------EQUILIBRIUM FUNCTIONS-------------
double LatticeBoltzmann::feq(vector3D & Jprime0,vector3D & Eprime0,vector3D & B0,
			     double epsilonr0,double mur0,
			     int r,int p,int i,int j){
  double VdotJp=(v[p][i]*Jprime0),Epdote=(e[p][i][j]*Eprime0),Bdotb=(b[p][i][j]*B0),aux;
  if(r==0)
...
void LatticeBoltzmann::Start(void){
  ...
  for(ix=0;ix<Lx;ix++) //para cada celda
	...
	H0=H(B0,mur0); Jprime0=sigma0*Eprime0;
	...
	for(r=0;r<2;r++)
	  for(p=0;p<3;p++)
		...
		fnew[ix][iy][iz][r][p][i][j]=f[ix][iy][iz][r][p][i][j]=
		  feq(Jprime0,Eprime0,B0,epsilonr0,mur0,r,p,i,j);
      }
}
	...
	for(r=0;r<2;r++)
	  for(p=0;p<3;p++)
		...
		fnew[ix][iy][iz][r][p][i][j]=UmUTau*f[ix][iy][iz][r][p][i][j]
		  +UTau*feq(Jprime0,Eprime0,B0,epsilonr0,mur0,r,p,i,j);
      }
}
...
void LatticeBoltzmann::Print(void){
  ...
  for(iz=0;iz<Lz;iz++){
    ...
    Jprime0=Jprime(E0,prefactor0); Eprime0=Eprime(E0,Jprime0,epsilonr0); 
    //Print
    E2=norma2(Eprime0); B2=norma2(B0);
    cout<<iz<<" "<<0.5*(Epsilon0*epsilonr0*E2+B2/(Mu0*mur0))<<endl;
  }
}


int main(){
  LatticeBoltzmann GaussianPulse;
  int t, tmax=140;
  
  GaussianPulse.Start();
  GaussianPulse.ImposeFields(0);
  
  for(t=0;t<tmax;t++){
    GaussianPulse.Collision();
    GaussianPulse.ImposeFields(t);
    GaussianPulse.Advection();
  }
